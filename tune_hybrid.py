...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Hyperparameter search for Conv+Attention+LSTM RUL model.")
    parser.add_argument("--config", default="config/tune_hybrid.json", help="JSON config path.")
    parser.add_argument("--output-dir", default="outputs/tuning_hybrid", help="Output directory for ranking artifacts.")
    parser.add_argument("--models-root", default="models/tuning_hybrid", help="Models root for trial checkpoints.")
...
def run_train(fd: str, trial_cfg: dict[str, Any], model_dir: Path) -> None:
    cmd = [
        sys.executable,
        str(ROOT / "train_hybrid_sequence.py"),
        "--fd",
        fd,
        "--model-dir",
        str(model_dir),
...
def main() -> None:
    ...
    for fd in fd_list:
        ...
        print(f"=== HYBRID TUNING {fd}: {len(fd_trials)} trials ===")
        ...
        fd_csv = output_dir / f"hybrid_tuning_{fd}.csv"
        fd_json = output_dir / f"hybrid_tuning_{fd}.json"
    ...
    all_csv = output_dir / "hybrid_tuning_all_fd.csv"
    all_json = output_dir / "hybrid_tuning_all_fd.json"
